...
LEXICON = {
    ...
    "ИИ-сленг: «рамка» (кроме канцелярского «в рамках»)": [
        # смысла шаблоны не различают: «картина в рамке» тоже попадёт в отчёт;
        # форма «рамках» (канцелярское «в рамках») не задевается
        r"\bрамк(?!ах\b)\w*", r"\bрамок\b", r"дал[аи]? рамку", r"задал[аи]? рамку",
        r"\bдерж\w* рамку", r"\bмоя рамка\b", r"\bрамка так\w+",
        r"\bв (этой|такой|той) рамке\b", r"\bпод рамку\b", r"\bрамка обсужден",
    ],
    "ИИ-сленг: «держать» в типовых оборотах": [
        # «держать в курсе» сюда не попадает, но смысла шаблоны не различают:
        # прямое значение («держит картину в руках») тоже попадёт в отчёт
        r"\bдерж\w* объ[её]м", r"\bдерж\w* нагрузку", r"\bдерж\w* фокус",
        r"\bдерж\w* контекст", r"\bдерж\w* картину", r"\bдерж\w* границ",
        r"\bдерж\w* планку", r"\bдерж\w* темп", r"\bдерж\w* принцип",
        r"\bдерж\w* установку", r"\bдерж\w* линию", r"\bперестал\w* держать",
        r"\bне держит\b", r"\bдержится на\b",
    ],
}
